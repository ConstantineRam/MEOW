...
namespace SimpleSpritePackerEditor
{
    public class SPTools
    {
        ...
        public static Object[] GetDirectoryAssets(string path)
        {
            ...
            {
                assets.AddRange(GetDirectoryAssets(directory));
            }
        ...
        public static int ReplaceReferencesInProject(List<SPSpriteInfo> spriteInfoList, SPReferenceReplacerWindow.ReplaceMode replaceMode, bool spriteRenderersOnly)
        {
            ...
            Object.DestroyImmediate(GameObject.Find("SPAnimHolder_temp"));
        ...
        public static int ReplaceReferences(Component[] components, Sprite find, Sprite replaceWith, bool spriteRenderersOnly)
        {
            ...
            foreach (Object comp in components)
            {
                ...
                if (comp is SPAnimationHolder)
                {
                    for (int i = 0; i < (comp as SPAnimationHolder).clips.Count; i++)
                    {
                        ...
                        ObjectReferenceKeyframe[] keyFrames = AnimationUtility.GetObjectReferenceCurve((comp as SPAnimationHolder).clips[i], curveBinding);

                        if (keyFrames != null)
                            for (int j = 0; j < keyFrames.Length; j++)
                            {
                                if (keyFrames[j].value == find)
                                {
                                    keyFrames[j].value = replaceWith;
                                    count++;
                                }
                            }

                        AnimationUtility.SetObjectReferenceCurve((comp as SPAnimationHolder).clips[i], curveBinding, keyFrames);
                    }
                ...
                else
                {
                    ...
                    foreach (FieldInfo fieldInfo in fields)
                    {
                        ...
                        if (fieldInfo.FieldType.IsArray)
                        {
                            ...
                            bool changed = false;
                            Sprite[] newArray = new Sprite[fieldValueArray.Length];
                            //Debug.Log(newArray.GetType() + " : " + fieldValueArray.GetType());
        ...
        public static Component[] GetProjectPrefabComponents()
        {
            ...
            GameObject animHolderGO = new GameObject("SPAnimHolder_temp");
            SPAnimationHolder animHolder = animHolderGO.AddComponent<SPAnimationHolder>();
            ...
            foreach (string assetPath in assets)
            {
                ...
                if (assetPath.Substring(assetPath.Length - 5) == ".anim")
                {
                    AnimationClip anim = assetObj as AnimationClip;
                    animHolder.clips.Add(anim);
                }
            }

            result.Add(animHolder);
